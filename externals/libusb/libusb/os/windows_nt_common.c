...
static void get_windows_version(void)
{
	...
	switch (version)
	{
	case 0x50: windows_version = WINDOWS_2000;
		w = "2000";
		break;
	case 0x51: windows_version = WINDOWS_XP;
		w = "XP";
		break;
	case 0x52: windows_version = WINDOWS_2003;
		w = "2003";
		break;
	case 0x60: windows_version = WINDOWS_VISTA;
		w = (ws ? "Vista" : "2008");
		break;
	case 0x61: windows_version = WINDOWS_7;
		w = (ws ? "7" : "2008_R2");
		break;
	case 0x62: windows_version = WINDOWS_8;
		w = (ws ? "8" : "2012");
		break;
	case 0x63: windows_version = WINDOWS_8_1;
		w = (ws ? "8.1" : "2012_R2");
		break;
	case 0x64: windows_version = WINDOWS_10;
		w = (ws ? "10" : "2016");
		break;
	default:
		if (version < 0x50)
                                      ...
                                      struct usbi_transfer* itransfer, DWORD io_result, DWORD io_size)
{
	...
	case ERROR_FILE_NOT_FOUND:
		...
		break;
	default:
		usbi_err(ITRANSFER_CTX(itransfer), "detected I/O error %u: %s", (unsigned int)io_result,
...
init_exit: // Holds semaphore here
	if ((init_count == 1) && (r != LIBUSB_SUCCESS))
	{
		// First init failed?

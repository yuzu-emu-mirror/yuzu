...
static int winusbx_set_interface_altsetting(int sub_api, struct libusb_device_handle* dev_handle, int iface,
                                            int altsetting);
...
static int hid_set_interface_altsetting(int sub_api, struct libusb_device_handle* dev_handle, int iface,
                                        int altsetting);
...
static int composite_set_interface_altsetting(int sub_api, struct libusb_device_handle* dev_handle, int iface,
                                              int altsetting);
...
static bool get_devinfo_data(struct libusb_context* ctx,
                             HDEVINFO* dev_info, SP_DEVINFO_DATA* dev_info_data, const char* enumerator,
                             unsigned _index)
{
	if (_index == 0)
	{
...
static SP_DEVICE_INTERFACE_DETAIL_DATA_A* get_interface_details_filter(struct libusb_context* ctx,
                                                                       HDEVINFO* dev_info,
                                                                       SP_DEVINFO_DATA* dev_info_data, const GUID* guid,
...
static int auto_claim(struct libusb_transfer* transfer, int* interface_number, int api_type)
{
	...
	{
		// No serviceable interface was found
                       ...
                       uint8_t port_number, DEVINST devinst)
{
	...
	if (parent_dev != NULL)
	{
		...
		do
		{
			if (!DeviceIoControl(hub_handle, IOCTL_USB_GET_NODE_CONNECTION_INFORMATION_EX, &conn_info,
			                     sizeof(conn_info),
		...
		case 2: dev->speed = LIBUSB_SPEED_HIGH;
			break;
		case 3: dev->speed = LIBUSB_SPEED_SUPER;
			break;
		default:
                                  ...
                                  uint8_t bus_number, DEVINST devinst)
{
	...
	if (dev->bus_number == 0)
	{
		...
		priv->active_config = 1;
		priv->root_hub = true;
		if (sscanf(dev_id, "PCI\\VEN_%04hx&DEV_%04hx%*s", &priv->dev_descriptor.idVendor,
		           &priv->dev_descriptor.idProduct) != 2)
		{
	...
	for (pass = 0; ((pass < nb_guids) && (r == LIBUSB_SUCCESS)); pass++)
	{
		...
		for (i = 0; ; i++)
		{
			...
			if (pass != GEN_PASS)
			{
				...
				r = get_interface_details(ctx, *dev_info, &dev_info_data, guid_list[pass], &_index,
				                          &dev_interface_path);
			...
			default:
				...
				if ((!pSetupDiGetDeviceRegistryPropertyA(*dev_info, &dev_info_data, SPDRP_INSTALL_STATE,
				                                         NULL, (PBYTE)&install_state, sizeof(install_state), &size)) ||
			...
			if ((pass <= GEN_PASS) && (pass != HCD_PASS))
			{
			...
			case GEN_PASS:
				...
				if (!pSetupDiGetDeviceRegistryPropertyA(*dev_info, &dev_info_data, SPDRP_ADDRESS,
				                                        NULL, (PBYTE)&port_nr, sizeof(port_nr), &size) || (size !=
					sizeof(port_nr)))
...
static int winusb_get_config_descriptor(struct libusb_device* dev, uint8_t config_index, unsigned char* buffer,
                                        size_t len)
{
	struct winusb_device_priv* priv = _device_priv(dev);
...
static int winusbx_open(int sub_api, struct libusb_device_handle* dev_handle)
{
	...
	for (i = 0; i < USB_MAXINTERFACES; i++)
	{
		...
		{
			file_handle = CreateFileA(priv->usb_interface[i].path, GENERIC_WRITE | GENERIC_READ,
			                          FILE_SHARE_WRITE | FILE_SHARE_READ,
			                          NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
			if (file_handle == INVALID_HANDLE_VALUE)
			{
				usbi_err(ctx, "could not open device %s (interface %d): %s", priv->usb_interface[i].path, i,
				         windows_error_str(0));
				switch (GetLastError())
				{
...
static int winusbx_claim_interface(int sub_api, struct libusb_device_handle* dev_handle, int iface)
{
		...
		if (!WinUSBX[sub_api].Initialize(file_handle, &winusb_handle))
		{
			...
			default:
				...
				for (i = 0; ; i++)
				{
					...
					dev_interface_details = get_interface_details_filter(
						ctx, &dev_info, &dev_info_data, &GUID_DEVINTERFACE_LIBUSB0_FILTER, i, filter_path);
					...
					{
						file_handle = CreateFileA(filter_path, GENERIC_WRITE | GENERIC_READ,
						                          FILE_SHARE_WRITE | FILE_SHARE_READ,
	...
	else
	{
		...
		if (!HANDLE_VALID(winusb_handle))
		{
			...
			{
				usbi_warn(ctx, "failed to auto-claim interface 0 (required to claim %d with WinUSB): %s", iface,
				          windows_error_str(0));
				return LIBUSB_ERROR_ACCESS;
			}
		}
...
static int winusbx_submit_iso_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	if ((sub_api == SUB_API_LIBUSBK) || (sub_api == SUB_API_LIBUSB0))
	{
		...
		if (IS_XFERIN(transfer))
		{
			...
			ret = WinUSBX[sub_api].IsoReadPipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length,
			                                   overlapped, iso_context);
		}
		else
		{
			usbi_dbg("writing %d iso packets", transfer->num_iso_packets);
			ret = WinUSBX[sub_api].IsoWritePipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length,
			                                    overlapped, iso_context);
		}
...
#        define WINUSBX_CHECK_API_SUPPORTED(API)       \
		...
		{
			ret = WinUSBX[sub_api].QueryPipeEx(winusb_handle,
			                                   (UINT8)priv->usb_interface[current_interface].current_altsetting,
			                                   (UCHAR)idx, &pipe_info_ex);
			if (!ret)
			{
				usbi_dbg("Couldn't query interface settings for USB pipe with index %d. Error: %s", idx,
				         windows_error_str(0));
				return LIBUSB_ERROR_NOT_FOUND;
			}
		...
		if (IS_XFERIN(transfer))
		{
			...
			{
				usbi_dbg(
					"The length of isochronous buffer must be a multiple of the MaximumBytesPerInterval * 8 / Interval");
				return LIBUSB_ERROR_INVALID_PARAM;
			}
		}
		else
		{
			...
			for (idx = 0; idx < transfer->num_iso_packets; ++idx)
			{
				if ((size_should_be_zero && transfer->iso_packet_desc[idx].length != 0) ||
					(transfer->iso_packet_desc[idx].length != pipe_info_ex.MaximumBytesPerInterval && idx + 1 < transfer
						->num_iso_packets && transfer->iso_packet_desc[idx + 1].length > 0))
				{
					usbi_dbg(
						"Isochronous packets for OUT transfer with Microsoft WinUSB must be contiguous in memory.");
					return LIBUSB_ERROR_INVALID_PARAM;
				}
		...
		ret = WinUSBX[sub_api].RegisterIsochBuffer(winusb_handle, transfer->endpoint, transfer->buffer,
		...
		if (IS_XFERIN(transfer))
		{
			ret = WinUSBX[sub_api].ReadIsochPipeAsap(buffer_handle, 0, transfer->length,
			                                         !transfer_priv->iso_break_stream, transfer->num_iso_packets,
			                                         (PUSBD_ISO_PACKET_DESCRIPTOR)transfer->iso_packet_desc,
			                                         overlapped);
		}
		else
		{
			ret = WinUSBX[sub_api].WriteIsochPipeAsap(buffer_handle, 0, out_transfer_length,
			                                          !transfer_priv->iso_break_stream, overlapped);
		}
...
static int winusbx_submit_bulk_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	if (IS_XFERIN(transfer))
	{
		...
		ret = WinUSBX[sub_api].ReadPipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length, NULL,
		                                overlapped);
	}
	else
	{
		usbi_dbg("writing %d bytes", transfer->length);
		ret = WinUSBX[sub_api].WritePipe(winusb_handle, transfer->endpoint, transfer->buffer, transfer->length, NULL,
		                                 overlapped);
	}
	...
	if (transfer->type == LIBUSB_TRANSFER_TYPE_ISOCHRONOUS)
	{
		...
		else if (sub_api == SUB_API_WINUSB)
		{
			if (IS_XFERIN(transfer))
			{
				...
				PUSBD_ISO_PACKET_DESCRIPTOR usbd_iso_packet_desc = (PUSBD_ISO_PACKET_DESCRIPTOR)transfer->
					iso_packet_desc;
				for (i = 0; i < transfer->num_iso_packets; ++i)
				{
					int length = (i < transfer->num_iso_packets - 1)
						             ? (usbd_iso_packet_desc[i + 1].Offset - usbd_iso_packet_desc[i].Offset)
...
static int _hid_get_report_descriptor(struct hid_device_priv* dev, void* data, size_t* size)
{
	...
	/* usage page */
	d[i++] = 0x06;
	d[i++] = dev->usagePage & 0xFF;
	d[i++] = dev->usagePage >> 8;
	/* usage */
	d[i++] = 0x09;
	d[i++] = (uint8_t)dev->usage;
...
static int hid_open(int sub_api, struct libusb_device_handle* dev_handle)
{
	...
	for (i = 0; i < USB_MAXINTERFACES; i++)
	{
		...
		{
			hid_handle = CreateFileA(priv->usb_interface[i].path, GENERIC_WRITE | GENERIC_READ,
			                         FILE_SHARE_WRITE | FILE_SHARE_READ,
			                         NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_OVERLAPPED, NULL);
			/*
	...
	case LIBUSB_REQUEST_TYPE_STANDARD:
		...
		case LIBUSB_REQUEST_GET_DESCRIPTOR:
			r = _hid_get_descriptor(priv->hid, hid_handle, LIBUSB_REQ_RECIPIENT(setup->RequestType),
			                        (setup->Value >> 8) & 0xFF, setup->Value & 0xFF,
                                              ...
                                              int altsetting)
{
	...
	return priv->usb_interface[iface].apib->
	                                  set_interface_altsetting(priv->usb_interface[iface].sub_api, dev_handle, iface,
	                                                           altsetting);
}
...
static int composite_submit_bulk_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              submit_bulk_transfer(priv->usb_interface[current_interface].sub_api,
	                                                                   itransfer);
}
...
static int composite_submit_iso_transfer(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              submit_iso_transfer(priv->usb_interface[current_interface].sub_api,
	                                                                  itransfer);
}
...
static int composite_clear_halt(int sub_api, struct libusb_device_handle* dev_handle, unsigned char endpoint)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              clear_halt(priv->usb_interface[current_interface].sub_api, dev_handle,
	                                                         endpoint);
}
...
static int composite_abort_control(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              abort_control(priv->usb_interface[current_interface].sub_api,
	                                                            itransfer);
}
...
static int composite_abort_transfers(int sub_api, struct usbi_transfer* itransfer)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              abort_transfers(priv->usb_interface[current_interface].sub_api,
	                                                              itransfer);
}
...
static int composite_copy_transfer_data(int sub_api, struct usbi_transfer* itransfer, uint32_t io_size)
{
	...
	return priv->usb_interface[current_interface].apib->
	                                              copy_transfer_data(priv->usb_interface[current_interface].sub_api,
	                                                                 itransfer, io_size);
}

...
private:
    ...
    void FlushAndRemoveRegion(VAddr addr, std::size_t size) {
        ...
        const u64 page_end = addr_end >> PAGE_SHIFT_;
        for (u64 page = addr_begin >> PAGE_SHIFT_; page <= page_end; ++page) {
    ...
    CachedQuery* Register(VideoCore::QueryType type, VAddr cpu_addr, u8* host_ptr, bool timestamp) {
        ...
        const u64 page = static_cast<u64>(cpu_addr) >> PAGE_SHIFT_;
    ...
    CachedQuery* TryGet(VAddr addr) {
        const u64 page = static_cast<u64>(addr) >> PAGE_SHIFT_;
        const auto it = cached_queries.find(page);
        if (it == std::end(cached_queries)) {
            return nullptr;
        }
    ...
    static constexpr std::uintptr_t PAGE_SIZE_ = 4096;
    static constexpr unsigned PAGE_SHIFT_ = 12;

...
std::pair<std::string, std::string> Cookie::NextPair(const std::string& text, size_t& position,
    ...
    size_t semiColonPosition = text.find(';', position);
    if (semiColonPosition == std::string::npos)
        semiColonPosition = length; // no ';' means take everything to end of string

    size_t equalsPosition = text.find('=', position);
    if (equalsPosition == std::string::npos || equalsPosition > semiColonPosition) {
        if (isNameValue)
            return std::make_pair(
                std::string(),
                std::string()); //'=' is required for name-value-pair (RFC6265 section 5.2, rule 2)
        equalsPosition =
            semiColonPosition; // no '=' means there is an attribute-name but no attribute-value
    }

    std::string first = Common::StripSpaces(text.substr(position, equalsPosition - position));
    std::string second;
    int secondLength = semiColonPosition - equalsPosition - 1;
    if (secondLength > 0)
        second = Common::StripSpaces(text.substr(equalsPosition + 1, secondLength));

    position = semiColonPosition;
    return std::make_pair(first, second);
}

std::vector<Cookie> Cookie::ParseAllCookies(const std::string& cookiesString) {
...
std::vector<Cookie> Cookie::ParseCookie(const std::string& cookieString) {
    std::vector<Cookie> result;
    size_t position = 0;
    const size_t length = cookieString.size();
    while (position < length) {
        std::pair<std::string, std::string> field = NextPair(cookieString, position, true);
        ...
        position = AdvanceTo(cookieString, position);
        ...
        while (position < length) {
            ...
            case ';':
                ...
                field = NextPair(cookieString, position, false);
                ...
                if (field.first == "expires") {
                    ...
                    std::string dateString =
                        Common::StripSpaces(cookieString.substr(position, end - position));
                    position = end;
                    std::tm dt;
                    if (Cookie::ParseTimeString(dateString, &dt)) {
                    ...
                    if (!rawDomain.empty()) {
                        ...
                        std::smatch match;
                        if (std::regex_match(rawDomain, match, url_reg)) {
                            cookie.SetDomain(maybeLeadingDot + rawDomain);
                        } else {
                            // Validation failed
                            return result;
                        }
